...
class TestPromptGeneration(unittest.TestCase):
    ...
    def summarize_idea(self, sample_request):
        """사용자의 아이디어를 5단어 이내로 요약하는 함수"""
        # 사용자의 아이디어를 AI를 통해 요약
        idea = f"{sample_request.get('problem', {}).get('identifiedIssues', [''])[0]} " \
               f"{sample_request.get('solution', {}).get('coreElements', [''])[0]}"
        
        # AI를 통해 요약 요청
        summary_prompt = f"다음 아이디어를 5단어 이내로 요약해 주세요: {idea}"
        summary_response = generate_prompt({"text": summary_prompt})
        
        # 요약 결과에서 5단어 이내로 반환
        return ' '.join(summary_response.split()[:5])
...
def generate_prompt(request: Dict[str, Any]) -> str:
    ...
    결과 JSON 형식:
    {
        "marketAnalysis": {
            ...
            "limitations": { ... },
            "requiredTeam": [
                { "role": "AI 전문가", "tasks": "자연어 처리 모델 개발 및 성능 최적화", "competencies": ["자연어 처리", "머신러닝", "딥러닝", "Python", "TensorFlow/PyTorch"] },
                { "role": "백엔드 개발자", "tasks": "API 및 서버 인프라 구축, 데이터베이스 설계", "competencies": ["Python", "Django/Flask", "REST API 설계", "Database (SQL/NoSQL)", "AWS/GCP"] },
                { "role": "디자이너", "tasks": "UX/UI 디자인 및 프로토타이핑", "competencies": ["UX/UI 디자인", "Figma/Sketch", "GUI 디자인", "사용자 리서치"] },
                { "role": "마케팅 전문가", "tasks": "시장 조사, 홍보 전략 수립 및 실행", "competencies": ["시장 조사", "데이터 분석", "디지털 마케팅", "콘텐츠 마케팅", "SEO/SEM"] }
            ],
